...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        u1.setId(UUID.nameUUIDFromBytes("Vini".getBytes()));
        u1.setName("vini");
        u1.setEmail("vini@email.com");
        u1.setPassword(passwordEncoder.encode("12345678"));

        u2.setId(UUID.nameUUIDFromBytes("Teste".getBytes()));
        u2.setName("teste");
        u2.setEmail("teste@email.com");
        u2.setPassword(passwordEncoder.encode("12345678"));

        u3.setId(UUID.nameUUIDFromBytes("Teste2".getBytes()));
        u3.setName("teste2");
        u3.setEmail("teste2@email.com");
        u3.setPassword(passwordEncoder.encode("12345678"));

        u1 = userUseCasePort.createUser(u1);
        u2 = userUseCasePort.createUser(u2);
        u3 = userUseCasePort.createUser(u3);


        t1.setId(UUID.nameUUIDFromBytes("Mesa Teste 1".getBytes()));
        t1.setTitle("mesa teste 1");
        t1.setAccessCode("fixed-access-code-1");
        t1.setCreatedAt(OffsetDateTime.now());
        t1.setRulebook("D&D");
        t1.setMaster(u1);

        t2.setId(UUID.nameUUIDFromBytes("Mesa Teste 2".getBytes()));
        t2.setTitle("mesa teste 2");
        t2.setAccessCode("fixed-access-code-2");
        ...
        c1.setId(UUID.nameUUIDFromBytes("Mago".getBytes()));
        c1.setName("mago");
        c1.setHealth(100);
        c1.setMana(100);
        c1.setStrength(10);
        ...
        c2.setId(UUID.nameUUIDFromBytes("Cavaleiro".getBytes()));
